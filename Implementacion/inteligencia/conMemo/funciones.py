...
def obtieneObjetoCerca (posx, posy, tipo, listaObjetos, listaP, indiceP,
    ...
    for i in range (tamano):
        ...
        # Si el tipo de objeto es cofre 
        if (tipo == 3):

            # Obtencion del identificador del objeto cofre
            id = listaObjetos[i].obtieneAtributo ('id')
                
            # Comprobacion de la utilidad del item
            if (itemUtil (id, listaItem, listaP, indiceP) == 0):
                tipoObjeto = -1
...
# En ese caso devuelve 1
# En caso contrario devuelve 0
def hayObjetoLejos (tipo, listaObjetos, listaP, indiceP, listaItem):
...
# Funcion que comprueba si el identificador dado se encuentra en la lista de
# items y si ademas, dicho objeto es util para el personaje o no
# Devuelve 1 si el item no esta en la lista o es util
# En caso contrario devuelve 0
def itemUtil (id, listaItem, listaP, indiceP):

    # Obtencion del tamano de la lista de items
    tamano = len (listaItem)

    # Busqueda por la lista de items
    for i in range (tamano):

        # Obtencion del identificador del item
        id1 = listaItem[i].obtieneAtributo ('id')

        # Si coinciden los identificadores
        if (id1 == id):

            # Obtencion del tipo de elemento
            elem = listaItem[i].obtieneAtributo ('elem')

            # Comprobacion de la utilidad del elemento
            utilidad = elementoUtil (elem, listaP, indiceP)
...
# Funcion que inserta un nuevo item en la lista limitada de items
def insertaItem (id, elem, listaItem):

    # Obtencion del tamano de items
    tamano = len (listaItem)

    # Si es el tamano maximo
    if (tamano == 5):
    ...
    nuevoItem = Item ()
    nuevoItem.reAsignaAtributo ('id', id)
    nuevoItem.reAsignaAtributo ('elem', elem)

    # Insercion del item en la lista
    listaItem.append (nuevoItem)
...
def compruebaPrec (prec, listaMundo, listaEdificios, listaObjetos, listaP,
                   indiceP, listaCasillas, listaItem):
...
def compruebaRegla (regla, listaMundo, listaEdificios, listaObjetos, listaP,
    ...
    verdadero = compruebaPrec (prec, listaMundo, listaEdificios, listaObjetos,
                               listaP, indiceP, listaCasillas, listaItem)
...
def realizaAccion (accion, listaMundo, listaEdificios, listaObjetos,
                   listaP, indiceP, listaCasillas, sockCliente, listaItem,
                   unObjetivo):
    ...
    # Accion: AbrirCofre
        ...
        # Obtencion del elemento del objeto cofre
        elem = listaObjetos[indiceO].obtieneAtributo ('equ')
        
        # Obtencion de la utilidad del elemento
        utilidad = elementoUtil (elem, listaP, indiceP)
        
        # Si no es util
        if (utilidad == 0):
            
            # Obtencion del identificador del objeto
            id = listaObjetos[indiceO].obtieneAtributo ('id')
            
            # Insercion del objeto en la lista de items
            insertaItem (id, elem, listaItem)
    ...
    # Acciones de movimiento
        ...
        # Accion: MoverHaciaPosada
            ...
            # Si no hay posada se utilizan las coordenas almacenadas
            if ((coorEdificio == []) and (unObjetivo != None)):
                destx = unObjetivo.obtieneAtributo ('posx')
                desty = unObjetivo.obtieneAtributo ('posy')
        ...
        # Accion: MoverHaciaBiblioteca
            ...
            # Si no hay biblioteca se utilizan las coordenas almacenadas
            if ((coorEdificio == []) and (unObjetivo != None)):
                destx = unObjetivo.obtieneAtributo ('posx')
                desty = unObjetivo.obtieneAtributo ('posy')
        ...
        # Accion: MoverHaciaArmas
            ...
            # Si no hay tienda de armas se utilizan las coordenas almacenadas
            if ((coorEdificio == []) and (unObjetivo != None)):
                destx = unObjetivo.obtieneAtributo ('posx')
                desty = unObjetivo.obtieneAtributo ('posy')
        ...
        # Accion: MoverHaciaEscudos
            ...
            # Si no hay tienda de escudos se utilizan las coordenas almacenadas
            if ((coorEdificio == []) and (unObjetivo != None)):
                destx = unObjetivo.obtieneAtributo ('posx')
                desty = unObjetivo.obtieneAtributo ('posy')
        ...
        # Accion: MoverHaciaArmaduras
            ...
            # Si no hay tienda de armaduras se utilizan las coordenas
            # almacenadas
            if ((coorEdificio == []) and (unObjetivo != None)):
                destx = unObjetivo.obtieneAtributo ('posx')
                desty = unObjetivo.obtieneAtributo ('posy')
        ...
        # Accion: MoverHaciaBruja
            ...
            # Si no hay bruja se utilizan las coordenas almacenadas
            if ((indiceO == -1) and (unObjetivo != None)):
                destx = unObjetivo.obtieneAtributo ('posx')
                desty = unObjetivo.obtieneAtributo ('posy')
...
def tomaDecisionReglas (listaMundo, listaEdificios, listaObjetos, listaP,
                        indiceP, listaCasillas, turno, sockCliente, listaItem):
...
# condiciones
def obtieneObjetivo (objetivoActual, listaEdificios, listaObjetos, listaP,
                     indiceP, listaCasillas, listaItem):

    # Obtencion del identificador del objetivo    
    if (objetivoActual != None):
        objetivo = objetivoActual.obtieneAtributo ('id')
    else:
        objetivo = 0        
    ...
    if (ham < maxham/2):
        ...
        objetivoActual.reAsignaAtributo ('id', 1)
    ...
    if ((sed < maxsed/3) and ((objetivo >= 2) or (objetivo == 0))):
        ...
        # Obtencion del objetivo
        objetivoActual = Objetivo ()
        objetivoActual.reAsignaAtributo ('id', 2)
        return objetivoActual
    ...
    if ((ene < maxene/2) and ((objetivo >= 3) or (objetivo == 0))):
        ...
        # Obtencion del objetivo
        objetivoActual = Objetivo ()
        objetivoActual.reAsignaAtributo ('id', 3)
        return objetivoActual
    ...
    if (edif == -1):
        ...
        # Si el objetivo es distinto de 4
        if (objetivo != 4):
        ...
        # Si el objetivo ya es 4
        else:
            coincide = 1
            
    # Si esta en el interior de algun edificio
    else:
        if (objetivo == 4):
            coincide = 1
        else:
    ...
    if ((pg != maxpg) and (coincide >= 1) and
        ...
        if (coincide == 2):
            ...
            # Obtencion de las coordenadas de la posada
            coorEdificio = obtieneEdificioLejos (posx, posy, 1,
                                                 listaEdificios, listaP)
            
            # Obtencion del objetivo
            objetivoActual = Objetivo ()
            objetivoActual.reAsignaAtributo ('id', 4)
            objetivoActual.reAsignaAtributo ('posx', coorEdificio[0])
            objetivoActual.reAsignaAtributo ('posy', coorEdificio [1])
    ...
    if ((pg != maxpg) and ((objetivo >= 5) or (objetivo == 0))):
        ...
        # Obtencion del objetivo
        objetivoActual = Objetivo ()
        objetivoActual.reAsignaAtributo ('id', 5)
        return objetivoActual
    ...
    if (edif == -1):
        ...
        # Si el objetivo es distinto de 6
        if (objetivo != 6):
    ...
    if (((coincide >= 1) or (obj != 0)) and
        ...
        if (coincide == 2):
            ...
            # Obtencion del indice del objeto bruja
            indiceO = obtieneObjetoLejos (posx, posy, 4, listaObjetos,
                                          listaP, indiceP, listaItem)
            
            # Obtencion de las coordenadas del objeto bruja
            posx = listaObjetos[indiceO].obtieneAtributo ('posx')
            posy = listaObjetos[indiceO].obtieneAtributo ('posy')
            
            # Obtencion del objetivo
            objetivoActual = Objetivo ()
            objetivoActual.reAsignaAtributo ('id', 6)
            objetivoActual.reAsignaAtributo ('posx', posx)
            objetivoActual.reAsignaAtributo ('posy', posy)
    ...
    if (edif == -1):
        ...
        # Si el objetivo es distinto de 7
        if (objetivo != 7):

            # Comprobacion de bibliotecas
            coincide = hayEdificioLejos (2, listaEdificios, listaP, indiceP)

            # Si hay una biblioteca
            if (coincide == 1):
                coincide = 2
                
        # Si el objetivo ya es 7
        else:
            coincide = 1
    ...
    if ((coincide >= 1) and ((objetivo >= 7) or (objetivo == 0))):
        ...
        if (coincide == 2):
            ...
            # Obtencion de las coordenadas de la biblioteca
            coorEdificio = obtieneEdificioLejos (posx, posy, 2,
                                                 listaEdificios, listaP)
            
            # Obtencion del objetivo
            objetivoActual = Objetivo ()
            objetivoActual.reAsignaAtributo ('id', 7)
            objetivoActual.reAsignaAtributo ('posx', coorEdificio[0])
            objetivoActual.reAsignaAtributo ('posy', coorEdificio [1])
    ...
    if (edif == -1):
        ...
        # Si el objetivo es distinto de 8
        if (objetivo != 8):

            # Comprobacion de tiendas de armas
            coincide = hayEdificioLejos (3, listaEdificios, listaP, indiceP)

            # Si hay una tienda de armas
            if (coincide == 1):
                coincide = 2
                
        # Si el objetivo ya es 8
        else:
            coincide = 1
    ...
    if ((coincide >= 1) and (mmr < 10) and (car < 5) and (arma < 4) and
        ...
        if (coincide == 2):
            ...
            # Obtencion de las coordenadas de la tienda de armas
            coorEdificio = obtieneEdificioLejos (posx, posy, 3,
                                                 listaEdificios, listaP)
            
            # Obtencion del objetivo
            objetivoActual = Objetivo ()
            objetivoActual.reAsignaAtributo ('id', 8)
            objetivoActual.reAsignaAtributo ('posx', coorEdificio[0])
            objetivoActual.reAsignaAtributo ('posy', coorEdificio [1])
    ...
    if (edif == -1):
        ...
        # Si el objetivo es distinto de 9
        if (objetivo != 9):

            # Comprobacion con tiendas de escudos
            coincide = hayEdificioLejos (4, listaEdificios, listaP, indiceP)
            
            # Si hay una tienda de escudos
            if (coincide == 1):
                coincide = 2
                
        # Si el objetivo ya es 9
        else:
            coincide = 1
    ...
    if ((coincide == 1) and (mmr < 10) and (car < 5) and (escudo < 3) and
        ...
        if (coincide == 2):
            ...
            # Obtencion de las coordenadas de la tienda de escudos
            coorEdificio = obtieneEdificioLejos (posx, posy, 4,
                                                 listaEdificios, listaP)
            
            # Obtencion del objetivo
            objetivoActual = Objetivo ()
            objetivoActual.reAsignaAtributo ('id', 9)
            objetivoActual.reAsignaAtributo ('posx', coorEdificio[0])
            objetivoActual.reAsignaAtributo ('posy', coorEdificio [1])
    ...
    if (edif == -1):
        ...
        # Si el objetivo es distinto de 10
        if (objetivo != 10):

            # Comprobacion con tiendas de armaduras
            coincide = hayEdificioLejos (5, listaEdificios, listaP, indiceP)

            # Si hay una tienda de armaduras
            if (coincide == 1):
                coincide = 2
                
        # Si el objetivo ya es 10
        else:
            coincide = 1
    ...
    if ((coincide == 1) and (car < 5) and (armadura < 4) and
        ...
        if (coincide == 2):
            ...
            # Obtencion de las coordenadas de la tienda de armaduras
            coorEdificio = obtieneEdificioLejos (posx, posy, 5,
                                                 listaEdificios, listaP)
            
            # Obtencion del objetivo
            objetivoActual = Objetivo ()
            objetivoActual.reAsignaAtributo ('id', 10)
            objetivoActual.reAsignaAtributo ('posx', coorEdificio[0])
            objetivoActual.reAsignaAtributo ('posy', coorEdificio [1])
...
def strips (postObj, listaOperadores, listaMundo, listaEdificios,
            listaObjetos, listaP, indiceP, listaCasillas, listaItem):
        ...
        # Si el tope es una precondicion
            ...
            # Si no es la precondicion vacio
                ...
                if (incluido == 0):
                    ...
                    # Si la precondicion es falsa
                    if (esVerdadero == 0):

                        # Utilizacion del objetivo 
                        if ((precond == 'tieneLejosPosada') and
                            (postObj == 'curado')):
                            esVerdadero = 1

                        elif ((precond == 'tieneLejosBruja') and
                              (postObj == 'incrementadasCarac')):
                            esVerdadero = 1

                        elif ((precond == 'tieneLejosBiblioteca') and
                              (postObj == 'incrementadosMH')):
                            esVerdadero = 1

                        elif ((precond == 'tieneLejosArmas') and
                              (postObj == 'tieneArma')):
                            esVerdadero = 1

                        elif ((precond == 'tieneLejosEscudos') and
                              (postObj == 'tieneEscudo')):
                            esVerdadero = 1

                        elif ((precond == 'tieneLejosArmaduras') and
...
def tomaDecisionStrips (objetivoActual, listaOperadores, listaMundo,
                        ...
                        listaCasillas, turno, sockCliente, listaItem):
